...
import MobileControlsManager from './MobileControls.js'

// Check if achievement tracker is available
if (!window.achievementTracker) {
  console.warn('AchievementTracker not loaded - achievements will not be recorded')
  // Create a dummy tracker to prevent errors
  window.achievementTracker = {
    ...
    createPlayerStats: async () => null,
    playerStatsObjectId: null,
    sessionKills: 0,
    sessionDamage: 0,
    pendingMints: [],
  }
...
const ZOMBIE_SCREAM_RADIUS = 15 // How far the scream travels
const ZOMBIE_WAKE_DELAY = 0.5 // Delay before other zombies wake up

// Add scream sound
const zombieScreamSound = new Audio('/sounds/zombie_scream.mp3')
zombieScreamSound.volume = 0.8

// Add these near the top with other constants
const ZOMBIE_BLOOD_FRENZY_SPEED_MULTIPLIER = 3.0 // 3x faster after tasting blood
const ZOMBIE_BLOOD_FRENZY_DURATION = 8.0 // How long the frenzy lasts in seconds
...
// Add movement variables for mobile
let mobileMoveX = 0
let mobileMoveZ = 0

let gameCompleted = false
let currentCompletedLevel = 0
...
function init() {
  ...
  // Check if achievement tracker is initialized
  // async function initAchievements() {
  //   if (window.achievementTracker && window._suiClient && window._playerStatsObjectId) {
  //     await window.achievementTracker.initialize(window._suiClient, window._currentAddress, window._playerStatsObjectId)
  //     console.log('Achievement tracker initialized with Sui client')
  //   }
  // }
  ...
  // After camera is created and controls are set up, initialize mobile controls
  if (isMobile) {
    // Disable pointer lock on mobile
    if (controls) {
      // Override pointer lock to prevent it from activating on mobile
    ...
    mobileControls = new MobileControlsManager(
      camera,
      (action, fade, loop, lock) => playAction(action, fade, loop, lock),
      (direction) => {
        mobileMoveX = direction.x
        mobileMoveZ = direction.z
      },
    )
  }
  ...
  loadingManager.onError = (url) => {
    ...
    // Don't block the game if a single asset fails to load
    // You might want to show a message but continue
    const loadingScreen = document.getElementById('loadingScreen')
    if (loadingScreen) {
      // Optionally keep loading screen visible or hide it
      loadingScreen.style.display = 'none'
    }
    isLevelReady = true // Still allow game to start even if some assets fail
  }
...
function injectGameUI() {
  ...
  if (document.getElementById('loadingScreen')) {
    console.log('Game UI already injected')
    return
  }
  ...
  const ui = `
        ...
        <div id="elevatorTimer" style="display: none;"></div>

        <div id="blocker">
            <div id="instructions">
                <span>
                    <span id="clickToPlay">Click to play</span><br />
                    W A S D — Move &nbsp;|&nbsp; SPACE — Jump<br />
                    SHIFT — Sprint &nbsp;|&nbsp; CTRL — Crouch<br />
                    E — Punch / Interact &nbsp;|&nbsp; P — Pause
                </span>
            </div>
        </div>
        ...
        <div id="gameOverOverlay" style="display: none;">
            <h1>Game Over</h1>
            <p class="reason"></p>
            <button id="game-over-retry-btn">Try Again</button>
        </div>

        <div id="levelDisplay">Floor: <span id="currentFloor">1</span>/5</div>

        <div id="objectiveDisplay"></div>

        <div id="healthVignette"></div>

        <div id="weaponDisplay">👊 Fists</div>

        <button id="game-back-btn">← Menu</button>
    `
  ...
  console.log('Game UI injected')

  // Wire up buttons after they're created
  const retryBtn = document.getElementById('game-over-retry-btn')
  if (retryBtn) {
    retryBtn.addEventListener('click', () => {
      const overlay = document.getElementById('gameOverOverlay')
      if (overlay) overlay.style.display = 'none'
      if (window._currentLevel && window.startGameFromLevel) {
        window.startGameFromLevel(window._currentLevel)
      } else {
        location.reload()
      }
    })
...
// Add this function to check and handle zombie collisions
function handleSleepingZombieCollision() {
  ...
  for (const zombie of zombies) {
    // Only trigger for sleeping zombies that are alive
    if (!zombie.isSleeping || zombie.isDead) continue

    // Get zombie hitbox position
    const zombiePos = zombie.model.position.clone()
    zombiePos.y += 0.5 // Center of zombie body

    // Calculate distance between player and zombie
    const dist = playerPos.distanceTo(zombiePos)

    // If player touches the sleeping zombie
    if (dist < PLAYER_HITBOX_RADIUS + ZOMBIE_RADIUS) {
      triggerZombieScream(zombie)
      return true // Only trigger one scream at a time
    }
  }
  return false
}

// Add this function to trigger zombie scream
function triggerZombieScream(sourceZombie) {
  // Prevent multiple screams from the same zombie
  if (sourceZombie.hasScreamed) return
  sourceZombie.hasScreamed = true

  console.log('Zombie screams!')

  // Play scream sound
  zombieScreamSound.currentTime = 0
  zombieScreamSound.play().catch((e) => console.log('Scream sound error:', e))
  ...
  // Show message
  showMessage('You startled a zombie! It screams!')

  // Camera shake
  shakeCamera(0.1, 200)

  // Wake up the screaming zombie
  if (sourceZombie.isSleeping) {
    sourceZombie.isSleeping = false
    sourceZombie.alerted = true
    sourceZombie.state = 'CHASE'
    sourceZombie.lastSeenTime = performance.now() / 1000
    sourceZombie.lastSeenPos.copy(controls.getObject().position)

    // Switch animation - use Crawl for crawlers, Walk for normal zombies
    const animName = sourceZombie.type === 'CRAWLER' ? 'Crawl' : 'Walk'
    if (sourceZombie.actions[animName]) {
      sourceZombie.currentAction?.fadeOut(0.4)
      sourceZombie.actions[animName].reset().setLoop(THREE.LoopRepeat).fadeIn(0.4).play()
      sourceZombie.currentAction = sourceZombie.actions[animName]
    }
  }

  // Wake up nearby zombies after delay
  const screamPos = sourceZombie.model.position.clone()

  setTimeout(() => {
    let wokenCount = 0
    zombies.forEach((zombie) => {
      if (zombie.isDead) return
      if (!zombie.isSleeping) return
      if (zombie === sourceZombie) return

      const dist = zombie.model.position.distanceTo(screamPos)
      if (dist <= ZOMBIE_SCREAM_RADIUS) {
        // Wake up this zombie
        zombie.isSleeping = false
        zombie.alerted = true
        zombie.state = 'CHASE'
    ...
    if (wokenCount > 0) {
      showMessage(`${wokenCount} zombies wake up!`)
    }
  }, ZOMBIE_WAKE_DELAY * 1000)
}

function triggerBloodFrenzy(zombie) {
  if (zombie.isBloodFrenzy) return

  const currentTime = performance.now() / 1000

  if (!zombie.originalSpeed) {
    zombie.originalSpeed = ZOMBIE_WALK_SPEED
  }

  zombie.isBloodFrenzy = true
  zombie.bloodFrenzyEndTime = currentTime + ZOMBIE_BLOOD_FRENZY_DURATION

  showMessage('The zombie tastes blood and becomes enraged!')

  // Add screen edge red flash
  const flash = document.createElement('div')
  flash.style.position = 'fixed'
  flash.style.top = '0'
  flash.style.left = '0'
  ...
  addBloodFrenzyEffect(zombie)

  // Play a sound effect for blood frenzy (optional)
  const frenzySound = new Audio('/sounds/zombie_frenzy.mp3')
  frenzySound.volume = 0.6
  frenzySound.play().catch(() => {})

  console.log(`Zombie entered blood frenzy! Speed increased ${ZOMBIE_BLOOD_FRENZY_SPEED_MULTIPLIER}x`)
}
...
function killZombie(zombie) {
  ...
  if (zombie.isBoss) {
    ...
    escapeMode = true
    window._bossKilled = true
  }

  // Use window.achievementTracker
  if (window.achievementTracker) {
    window.achievementTracker.recordKill(zombie.isBoss ?? false)
  }
}
...
function endGame(reason, isVictory = false) {
  ...
  const gameOverEvent = new CustomEvent('gameOver', {
    detail: {
      reason: reason,
      isVictory: isVictory,
      finalLevel: currentLevelIndex,
    },
  })
  window.dispatchEvent(gameOverEvent)
  console.log('💀 Dispatched gameOver event:', { reason, isVictory })
  ...
  if (overlay) {
    ...
    if (isVictory) {
      ...
      if (reasonEl) reasonEl.textContent = reason || 'You successfully escaped!'

      // Track completed level
      currentCompletedLevel = currentLevelIndex
    ...
    // Add return to menu button if not already there
    const existingBtn = overlay.querySelector('.return-menu-btn')
    if (!existingBtn) {
      const returnBtn = document.createElement('button')
      returnBtn.className = 'return-menu-btn'
      returnBtn.textContent = 'Return to Menu'
      returnBtn.style.marginTop = '10px'
      returnBtn.style.background = '#666'
...
function saveProgress() {
  ...
  let progress = savedProgress ? JSON.parse(savedProgress) : { highestLevel: 0 }

  if (currentCompletedLevel + 1 > progress.highestLevel) {
    progress.highestLevel = currentCompletedLevel + 1
    localStorage.setItem('gameProgress', JSON.stringify(progress))

    // Dispatch event to update React UI
    window.dispatchEvent(
      new CustomEvent('gameProgressUpdated', {
        detail: { highestLevel: progress.highestLevel },
      }),
    )
  }
}
...
async function returnToMenu() {
  ...
  // Use window.achievementTracker
  if (window.achievementTracker) {
    console.log('Waiting 5 seconds for floor results to be processed...')
    // Wait a bit for any pending transactions
    await new Promise(resolve => setTimeout(resolve, 5000))
        await window.achievementTracker.refreshPlayerStats()
...
//     endGame('You Escaped the Facility! Congratulations!', true)
//     // Hide mobile controls on final completion
//     if (isMobile && mobileControls) {
//       mobileControls.hide()
//     }
//     return
...
// Update advanceToNextLevel function
async function advanceToNextLevel() {
    console.log('🎮 advanceToNextLevel - currentLevelIndex:', currentLevelIndex)
    
    const floorTimeMs = window.achievementTracker?.floorStartTime
        ? Math.round(performance.now() - window.achievementTracker.floorStartTime)
        : 0

    // Submit floor result - currentLevelIndex should be 0-5
    if (window.achievementTracker && window.achievementTracker._isReady()) {
        console.log('📤 Submitting floor result for floor index:', currentLevelIndex)
        await window.achievementTracker._submitFloorResult(
            currentLevelIndex,  // This should be 0, 1, 2, 3, 4, or 5
            floorTimeMs,
            window._bossKilled || false
        )
    }
    ...
    window.dispatchEvent(
        new CustomEvent('floorComplete', {
            detail: {
                floorIndex: currentLevelIndex,
                bossKilled: window._bossKilled || false,
                survivedWaves: currentLevelIndex + 1,
                floorTimeMs,
            },
        }),
    )
    ...
    if (currentLevelIndex >= LEVEL_ORDER.length - 1) {
        endGame('Congratulations! You escaped the facility!', true)
        if (isMobile && mobileControls) mobileControls.hide()
        return
    }

    endGame(`Level ${currentLevelIndex + 1} Complete!`, true)
}

// Update victory condition when completing a level
function checkLevelComplete() {
  // Check if current level objective is complete
  if (objectiveManager.current?.completed) {
    advanceToNextLevel()
  }
}
...
//   // Show mobile controls when level starts
//   if (isMobile && mobileControls) {
//     mobileControls.show()
//   }

//   // Check if essential game components exist
//   if (!controls || !levelLoader) {
//     console.error('Game not fully initialized yet. Cannot load level.')
//     return
//   }
...
function loadLevel(levelIndex = null) {
  ...
  window._currentLevelIndex = currentLevelIndex
    ...
    if (levelId === 'level4') {
      ...
      if (crawlerReady && zombie1Template) {
        ...
        for (let i = 0; i < 50; i++) {
          ...
          z2.hasScreamed = false // Initialize scream flag
...
//   // Check for collision with sleeping zombies AFTER movement
//   handleSleepingZombieCollision()
...
function handleFPSMovement() {
  ...
  if (isMobile && mobileControls && mobileControls.isActive) {
    // Use mobile touch input
    moveX = mobileMoveX
    moveZ = mobileMoveZ

    // Set movement flags based on direction
    moveForward = moveZ > 0.1
    moveBackward = moveZ < -0.1
    moveLeft = moveX < -0.1
    moveRight = moveX > 0.1
  } else {
    // Use keyboard input
  ...
  // Vertical movement (jump/fall) - only on desktop or with jump button on mobile
  controls.getObject().position.y += velocity.y * delta
  if (controls.getObject().position.y < (crouch ? crouchHeight : standHeight)) {
    velocity.y = 0
    controls.getObject().position.y = crouch ? crouchHeight : standHeight
  ...
  // Animation state
  if (!isMobile && (!controls.isLocked || !actions['rig|Idle'] || animationLocked)) return

  if (moveForward || moveBackward || moveLeft || moveRight || (isMobile && (moveX !== 0 || moveZ !== 0))) {
    if (sprint && !isMobile) {
      playAction('rig|Sprint_Type_1', 0.15)
    } else {
      playAction('rig|Walk', 0.15)
...
function tryMove(moveVec) {
  ...
  // Check collision with sleeping zombies
  const newPos = camPos.clone().add(moveVec)
  for (const zombie of zombies) {
    if (!zombie.isSleeping || zombie.isDead) continue

    const zombiePos = zombie.model.position.clone()
    const dist = newPos.distanceTo(zombiePos)

    // If would collide with sleeping zombie
    if (dist < PLAYER_RADIUS + ZOMBIE_RADIUS) {
      // Trigger scream and return without moving
      triggerZombieScream(zombie)
      return
    }
  }
...
function updateZombies(delta) {
  ...
  zombies.forEach((zombie) => {
    ...
    if (zombie.type === 'CRAWLER' && zombie.state === 'PATROL') {
      ...
      // Calculate patrol speed with frenzy multiplier
      let patrolSpeed = ZOMBIE_PATROL_SPEED
      if (zombie.isBloodFrenzy) {
        patrolSpeed = ZOMBIE_PATROL_SPEED * ZOMBIE_BLOOD_FRENZY_SPEED_MULTIPLIER
      }

      // Move back and forth around origin
      zombie.model.position.add(zombie.patrolDir.clone().multiplyScalar(delta * patrolSpeed))
    ...
    // ─────────────────────────
    // CHASE MOVEMENT (with Blood Frenzy)
    // ─────────────────────────
    ...
    if (zombie.state === 'CHASE' && (dist < ZOMBIE_MEMORY_DISTANCE || timeSinceSeen < ZOMBIE_LOSE_INTEREST_TIME)) {
      ...
      // Determine base animation and speed based on zombie type
      let moveAnim = 'Walk'
      let baseMoveSpeed = ZOMBIE_WALK_SPEED

      if (zombie.type === 'NORMAL') {
        moveAnim = 'Run'
        baseMoveSpeed = ZOMBIE_RUN_SPEED
      } else if (zombie.type === 'CRAWLER') {
        moveAnim = 'Crawl'
        baseMoveSpeed = ZOMBIE_WALK_SPEED
      }

      // Apply blood frenzy speed multiplier if active
      let moveSpeed = baseMoveSpeed
      if (zombie.isBloodFrenzy) {
        moveSpeed = baseMoveSpeed * ZOMBIE_BLOOD_FRENZY_SPEED_MULTIPLIER
        // For crawlers, keep using Crawl animation but make it faster
        if (zombie.type === 'CRAWLER') {
          moveAnim = 'Crawl' // Keep crawl animation for crawlers
        } else {
          moveAnim = 'Run' // Force run animation for normal zombies during frenzy
        }
      }
      ...
      // Keep zombie grounded and facing player
      zombie.model.position.y = 0
      zombie.model.lookAt(playerPos.x, zombie.model.position.y, playerPos.z)
    ...
    // ─────────────────────────
    // LOSE INTEREST - RETURN TO PATROL
    // ─────────────────────────
...
function handleZombieAttack(zombie, playerPos) {
  ...
  // TRIGGER BLOOD FRENZY if player was damaged and zombie is alive
  if (playerHealth > 0 && !zombie.isDead && zombie.type === 'CRAWLER') {
    triggerBloodFrenzy(zombie)
  }

  if (playerHealth <= 0) return
}

// Add visual effect for blood frenzy
function addBloodFrenzyEffect(zombie) {
  // Create a red glow material for the zombie
  if (zombie.model.children) {
    zombie.model.traverse((child) => {
      if (child.isMesh && child.material) {
        ...
        glowMaterial.color = new THREE.Color(0xff3333)
        glowMaterial.emissive = new THREE.Color(0xff0000)
        glowMaterial.emissiveIntensity = 0.1
        child.material = glowMaterial

        // Schedule restoration
        setTimeout(() => {
          if (child.userData.originalMaterial && !zombie.isBloodFrenzy) {
            child.material = child.userData.originalMaterial
          }
        }, ZOMBIE_BLOOD_FRENZY_DURATION * 1000)
      }
    })
  }
}

// Add function to update blood frenzy state
function updateBloodFrenzy(delta) {
  const currentTime = performance.now() / 1000

  zombies.forEach((zombie) => {
    if (zombie.isBloodFrenzy && currentTime >= zombie.bloodFrenzyEndTime) {
      // End blood frenzy
      zombie.isBloodFrenzy = false
      ...
      console.log('Zombie blood frenzy ended')
    }
  })
}
...
function applyPlayerDamage(amount) {
  ...
  // Use window.achievementTracker
  if (window.achievementTracker) {
    window.achievementTracker.recordDamage(amount)
  }
...
function animate() {
  ...
  if (!paused && isLevelReady) {
    ...
    // Update blood frenzy state
    updateBloodFrenzy(delta)
...
// Expose functions for mobile controls
window.getZombieInFront = getZombieInFront
window.damageZombie = damageZombie
...
window.stopGame = function () {
  ...
  // Remove all injected game UI elements
  const ids = [
    'loadingScreen',
    'elevatorTimer',
    'blocker',
...
// 5. In main.js — listen for floorComplete
window.addEventListener('floorComplete', (e) => {
  if (window.achievementTracker) {
    window.achievementTracker.onFloorComplete(e.detail.floorIndex, e.detail.bossKilled)
  }
})
